/* ============================================================================
   CommandCenterV2.tsx
   "Obsidian Weissach" — Healthcare Staffing Edition (v2.4 - Production Gold)
   
   Architecture:
   ├─ SYSTEM: Design tokens (anim, surface, type, geo)
   ├─ Core: Strict TypeScript, Hoisted Regex, Hybrid Scroll Physics
   ├─ Visuals: Premium Inline Thumbnails, Glass Tables, Milled Surfaces
   
   Refinements:
   - RESTORED: InlineImageThumbnail & InlineFilePill (Rich Media)
   - RESTORED: Premium Table Styling (Rounded, Glass, Hover)
   - FIXED: Tailwind dynamic class purging via static lookup maps
   - OPTIMIZED: Memoized Markdown tree & Layout-safe Input resizing
============================================================================ */
...
import {
    ...
    cn,
    triggerHaptic,
    copyToClipboard as systemCopyToClipboard,
    FilmGrain,
    OrbitalRadar,
    ToastProvider,
    useToast,
...
// ============================================================================
// 0. CONSTANTS & STATIC CONFIG (Hoisted for Performance)
// ============================================================================
...
// Status Pill Lookups (Static to prevent Tailwind Purge)
...
/**
 * Synchronous layout effect to prevent text area height jitter
 */
            ...
            <div className="rounded-2xl overflow-hidden border border-white/[0.08] bg-white/[0.03] hover:bg-white/[0.05] hover:border-indigo-500/30 transition-all shadow-lg">
                <div className="relative aspect-video bg-black/50">
                    <img
                        ...
                        onError={(e) => { (e.target as HTMLImageElement).src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><text y=".9em" font-size="90">🖼️</text></svg>'; }}
                    />
                    <div className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/60 border border-white/10 backdrop-blur flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <div className="px-3 py-2 flex items-center gap-2 bg-black/20">
                    ...
                    <span className="text-[11px] text-zinc-300 truncate">{cleanLabel}</span>
                </div>
            </div>
        </a>
    );
...
const CopyButton: FC<{ content: string }> = memo(({ content }) => {
    ...
    return (
        <button onClick={handleCopy} aria-label="Copy to clipboard" className={cn('p-1.5 rounded-md transition-all duration-200', copied ? 'text-indigo-400 bg-indigo-500/10' : 'text-zinc-600 hover:text-zinc-300 hover:bg-white/5')}>
...
const CandidateVerdict: FC<{ verdict: 'STRONG MATCH' | 'REVIEW NEEDED' | 'NOT A FIT'; details?: string }> = memo(({ verdict, details }) => {
    const config = useMemo(() => {
        switch (verdict) {
            case 'STRONG MATCH': return { dot: 'bg-emerald-500', text: 'text-emerald-500', border: 'border-emerald-500/20', bg: 'bg-emerald-500/10', label: 'Proceed', sub: 'Ready for Interview', icon: 'check' };
            case 'REVIEW NEEDED': return { dot: 'bg-amber-500', text: 'text-amber-500', border: 'border-amber-500/20', bg: 'bg-amber-500/10', label: 'Review', sub: 'Additional Screening', icon: '?' };
            case 'NOT A FIT':
            default: return { dot: 'bg-rose-500', text: 'text-rose-500', border: 'border-rose-500/20', bg: 'bg-rose-500/10', label: 'Pass', sub: 'Requirements Not Met', icon: 'x' };
        }
    }, [verdict]);
    ...
    return (
        <motion.div layout initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={SYSTEM.anim.fluid} className={cn('my-6 relative overflow-hidden rounded-[18px] bg-[#0A0A0B] shadow-2xl group select-none isolate', SYSTEM.surface.milled)}>
            ...
            <div className="px-6 py-3.5 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
                ...
                <div className={cn('flex items-center gap-2 px-2 py-0.5 rounded-full border', config.bg, config.border)}>
                    <span className={cn('w-1.5 h-1.5 rounded-full animate-pulse', config.dot)} />
                    <span className={cn('text-[9px] font-bold tracking-wider uppercase', config.text)}>{config.label}</span>
                </div>
            </div>
            <div className="relative p-6 flex items-start gap-4">
                <div className="flex-1">
                    <div className="text-2xl md:text-3xl font-medium text-white tracking-tight leading-none mb-1 tabular-nums">{verdict}</div>
                    {details && <div className="text-[13px] text-zinc-400 mt-3 leading-relaxed">{details}</div>}
                    <div className={cn('text-[10px] uppercase tracking-wider font-mono mt-3', config.text)}>{config.sub}</div>
                </div>
                <div className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center bg-white/[0.02]">
                    {config.icon === 'check' && <Check size={20} className="text-emerald-400" />}
                    {config.icon === '?' && <span className="text-amber-400 text-lg font-bold">?</span>}
                    {config.icon === 'x' && <X size={20} className="text-rose-400" />}
                </div>
            </div>
        </motion.div>
    );
    ...
    <motion.div layout initial={{ x: -5, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={SYSTEM.anim.fluid} className={cn('my-6 relative overflow-hidden rounded-r-[16px] border-l-[3px] border-l-indigo-500/80 shadow-[20px_0_40px_-10px_rgba(99,102,241,0.05)]', SYSTEM.surface.hud)}>
        <div className="p-5">
            <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest">{title}</span>
            </div>
            <div className="text-[14px] text-indigo-100/90 leading-[1.6] font-medium tracking-wide">{content}</div>
        </div>
    </motion.div>
));
...
const ThinkingPill: FC<{ onStop?: () => void; status?: 'thinking' | 'streaming' | 'grounding' }> = memo(({ onStop, status = 'thinking' }) => {
    ...
    useEffect(() => {
        if (status === 'thinking') {
            const interval = setInterval(() => setPhaseIndex(p => (p + 1) % RECRUITING_PHASES.length), 2200);
            return () => clearInterval(interval);
        }
    }, [status]);
    const displayText = status === 'streaming' ? 'LIVE STREAM' : status === 'grounding' ? 'SOURCING' : RECRUITING_PHASES[phaseIndex];
    return (
        <motion.div layout initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={SYSTEM.anim.fluid} className="absolute bottom-[100%] left-1/2 -translate-x-1/2 mb-6 z-30 flex items-center gap-3 px-4 py-2 rounded-full bg-[#050505] border border-white/10 shadow-2xl backdrop-blur-md">
            ...
            <AnimatePresence mode="wait">
                <motion.span key={displayText} initial={{ opacity: 0, filter: 'blur(4px)' }} animate={{ opacity: 1, filter: 'blur(0px)' }} exit={{ opacity: 0, filter: 'blur(4px)' }} className={cn(SYSTEM.type.mono, 'text-zinc-300 min-w-[120px] text-center')}>
                    {displayText}
                </motion.span>
            </AnimatePresence>
            {onStop && <button onClick={onStop} className="ml-1 text-zinc-600 hover:text-zinc-200 transition-colors p-1" aria-label="Stop"><Square size={10} fill="currentColor" /></button>}
        </motion.div>
    );
});
...
const SmartChips: FC<{ onSelect: (query: string) => void }> = memo(({ onSelect }) => (
    <div className="flex gap-2 overflow-x-auto pb-3 scrollbar-hide px-1">
        {[
            ...
            <motion.button key={chip.label} onClick={() => { triggerHaptic(); onSelect(chip.query); }} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04, ...SYSTEM.anim.fluid }} whileHover={{ scale: 1.02, y: -1, backgroundColor: 'rgba(255,255,255,0.06)' }} whileTap={{ scale: 0.98 }} className={cn('flex-shrink-0 flex items-center gap-2 px-3.5 py-2 bg-white/[0.03] border border-white/[0.08] transition-all backdrop-blur-sm', SYSTEM.geo.pill)}>
                <span className="text-zinc-400">{chip.icon}</span>
                <span className="text-[10px] font-medium text-zinc-300 tracking-wide uppercase">{chip.label}</span>
            </motion.button>
        ))}
    </div>
));
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={SYSTEM.anim.fluid} className={cn('rounded-[20px] overflow-hidden bg-white/[0.02] backdrop-blur-md border border-white/[0.08] shadow-[0_4px_24px_-4px_rgba(0,0,0,0.3)]')}>
            ...
            <div className="px-5 py-4 relative group">
                <div className={cn('flex-1 min-w-0 relative', !isExpanded && isLongBody && 'max-h-[280px] overflow-hidden')}>
                    <div className={cn(SYSTEM.type.body, 'text-[#C4C4C4] whitespace-pre-wrap leading-relaxed')}>{formattedBody.split(REGEX_EMAIL).map((part, i) => REGEX_EMAIL.test(part) ? <span key={i} className="text-indigo-400 cursor-pointer underline">{part}</span> : part)}</div>
...
// ============================================================================
// 5. MESSAGE BUBBLE & MARKDOWN COMPONENTS
// ============================================================================
...
const MessageBubble: FC<MessageBubbleProps> = memo(({ role, content, isStreaming, toolInvocations }) => {
    ...
    // Memoized Markdown Components (Critical for 60fps)
    const components: Components = useMemo(() => ({
        ...
        a: ({ href, children }) => {
            ...
            if (REGEX_NOVA_ID.test(label)) {
                return <a href={safeHref} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="inline-flex items-center gap-1 text-zinc-400 hover:text-indigo-400 font-mono text-[12px] group transition-colors">{label}<ExternalLink size={10} className="opacity-0 group-hover:opacity-100 transition-opacity" /></a>;
            }

            // 2. Attachment Link (Image/File)
            const cleanLabel = stripPaperclip(label);
            const looksLikeAttachment = label.includes('📎') || /\.[a-z0-9]{2,5}$/i.test(cleanLabel);

            if (looksLikeAttachment) {
                const ext = getFileExtension(safeHref, label);
                if (IMAGE_EXTENSIONS.has(ext)) return <InlineImageThumbnail href={safeHref} label={label} />;
                return <InlineFilePill href={safeHref} label={label} isPdf={ext === PDF_EXTENSION} />;
            }
        ...
        // Premium Table Styling (Restored)
        ...
        td: ({ children }) => {
            ...
            if (styleKey && STATUS_STYLES[styleKey]) {
                return <td className="px-4 py-4"><span className={cn('inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-medium border', STATUS_STYLES[styleKey])}>{flattenChildrenText(children)}</span></td>;
            }
    ...
    const renderContent = useMemo(() => {
        ...
        if (isUser) {
            ...
            while ((match = REGEX_ATTACHMENT.exec(content)) !== null) {
                attachments.push({ filename: match[1].trim(), url: match[2] });
            }
            if (attachments.length > 0) {
                const textContent = content.replace(REGEX_ATTACHMENT, '').trim();
                return <>{textContent && <p className={cn(SYSTEM.type.body, 'text-[#1a1a1a]')}>{textContent}</p>}{attachments.map((att, idx) => <UserAttachment key={idx} filename={att.filename} url={att.url} />)}</>;
            }
        }

        // Assistant: Special Artifacts
        const emailMatch = content.match(REGEX_EMAIL_HEADER);
        if (emailMatch) {
            const to = content.match(REGEX_EMAIL_TO)?.[1].trim();
            const subject = content.match(REGEX_EMAIL_SUBJECT)?.[1].trim();
            const bodyMatch = content.match(REGEX_EMAIL_BODY);
            const remaining = content.split('---').pop()?.replace(/^IMPORTANT[\s\S]*/, '').trim();
            if (subject) return <><EmailCard to={to} subject={subject} body={bodyMatch ? bodyMatch[1].trim() : ''} />{remaining && <div className="mt-4"><ReactMarkdown remarkPlugins={[remarkGfm]} components={components}>{remaining}</ReactMarkdown></div>}</>;
        }

        const verdictMatch = content.match(REGEX_VERDICT);
        if (verdictMatch) return <CandidateVerdict verdict={verdictMatch[1].toUpperCase() as any} details={content.replace(verdictMatch[0], '').trim()} />;

        const insightMatch = content.match(REGEX_INSIGHT);
        if (insightMatch) return <AssessmentHUD content={insightMatch[1].trim()} />;
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className={cn('rounded-[16px] overflow-hidden', SYSTEM.surface.panel)}>
            <button onClick={() => isComplete && setExpanded(!expanded)} disabled={!isComplete} className={cn('w-full px-4 py-3 flex items-center justify-between text-left transition-colors', isComplete && 'hover:bg-white/[0.02] cursor-pointer')}>
                <div className="flex items-center gap-3"><span className={cn(SYSTEM.type.mono, isComplete ? 'text-indigo-400' : 'text-zinc-500')}>{isComplete ? 'Result' : 'Running'}</span><span className={cn(SYSTEM.type.h1)}>{displayName}</span></div>
            ...
            <AnimatePresence>
                {expanded && isComplete && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        <div className="px-4 pb-4 border-t border-white/[0.04]"><pre className="text-xs font-mono text-zinc-400 overflow-x-auto pt-3 whitespace-pre-wrap">{JSON.stringify(result, null, 2)}</pre></div>
                    </motion.div>
                )}
            </AnimatePresence>
        </motion.div>
...
const InputDeck: FC<InputDeckProps> = memo(({ value, onChange, onSend, onStop, isProcessing, inputRef, attachments, onRemoveAttachment, isDragActive, isUploading, dragHandlers, handlePaste, triggerFileSelect, fileInputRef, onFilesSelected }) => {
    ...
    const handleKeyDown = (e: ReactKeyboardEvent) => {
        if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); if ((value.trim() || attachments.length > 0) && !isUploading) onSend(); }
        if (e.key === 'Escape' && isProcessing) { e.preventDefault(); onStop(); }
    };
    const canSend = (!!value.trim() || attachments.length > 0) && !isUploading;
    return (
        <motion.div layout className={cn('flex flex-col p-1.5 relative overflow-hidden transition-all duration-300 will-change-transform', SYSTEM.geo.input, 'bg-[#0A0A0B] shadow-2xl', SYSTEM.surface.milled, 'focus-within:border-indigo-500/30 focus-within:shadow-[0_0_20px_-5px_rgba(99,102,241,0.15)]', isDragActive && 'border-indigo-500/50 shadow-[0_0_30px_-5px_rgba(99,102,241,0.3)] scale-[1.01]')} transition={SYSTEM.anim.fluid} {...dragHandlers}>
                ...
                {attachments.length > 0 && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="px-2 pt-2">
                        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                            {attachments.map((att) => (
                                <div key={att.id} className="relative group flex-shrink-0 w-14 h-14 rounded-xl bg-white/5 border border-white/10 overflow-hidden">
...
export const CommandCenterV2: FC = () => {
    ...
    const { attachments, isDragActive, isUploading, addFiles, removeFile, clearAll: clearAttachments, dragHandlers, handlePaste, fileInputRef, triggerFileSelect } = useFileUpload({
        onUploadError: (err, file) => console.error(`Upload error: ${file.name}`, err),
    });
    ...
    // Hybrid Scroll: Instant snap for AI streaming, Smooth for User interactions
    ...
    useLayoutEffect(() => {
        ...
        if (isStreaming) el.scrollTop = el.scrollHeight; // Snap
        else requestAnimationFrame(() => el.scrollTo({ top: el.scrollHeight, behavior: 'smooth' })); // Physics
    ...
    const handleSend = useCallback(async (query?: string) => {
        ...
        const fileAttachments = attachments.filter(a => a.base64Data && (a.mimeType.startsWith('image/') || a.mimeType === 'application/pdf')).map(a => ({ base64: a.base64Data!, mimeType: a.mimeType, fileName: a.fileName }));
        let msg = text;
        if (attachments.length > 0) {
            const links = attachments.filter(a => a.publicUrl).map(a => `[📎 ${a.fileName}](${a.publicUrl})`).join('\n');
            if (links) msg = text ? `${text}\n\n${links}` : `Analyze:\n\n${links}`;
        }
        setInputValue(''); clearAttachments(); setShouldAutoScroll(true); triggerHaptic();
        await sendMessage(msg, fileAttachments.length ? fileAttachments : undefined);
        ...
        <ToastProvider>
            <LayoutGroup>
                <motion.div layoutId="chat" className={cn('fixed z-50 flex flex-col overflow-hidden isolate border border-white/[0.08] shadow-[0_40px_120px_-20px_rgba(0,0,0,0.9)]', SYSTEM.surface.void)} style={containerStyle}>
                    ...
                    <div ref={scrollRef} onScroll={handleScroll} className="relative flex-1 overflow-y-auto px-6 pt-4 pb-44 scroll-smooth no-scrollbar z-10 will-change-transform">
                        <AnimatePresence mode="popLayout">
                            {history.length === 0 ? (
                                ...
                                history.map((msg, i) => <MessageBubble key={i} role={msg.role} content={msg.content} isStreaming={isStreaming && i === history.length - 1 && msg.role === 'assistant'} toolInvocations={msg.toolInvocations} />)
                            )}
                        </AnimatePresence>
                    </div>
