...
import type { Click } from '../../shared/types/database';

interface ExtractorModalProps {
  isOpen: boolean;
  onClose: () => void;
  selectedProspects: Click[];
}
...
const ExtractorModal: React.FC<ExtractorModalProps> = ({ isOpen, onClose, selectedProspects }) => {
  ...
  const getEmailClientLink = (click: Click, template: any) => {
    const email = click.candidate_email;
        ...
        <div className="mt-8">
          ...
          {generatedContent && (
            <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2 -mr-2">
              {selectedProspects.map((click) => {
                const template = generatedContent.find((t: any) => t.jobId == click.job_id);
                if (!template) return null;

                return (
                  <div key={`${click.application_id}-${click.job_id}`} className="bg-gray-50 border border-gray-100 rounded-xl p-4">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <p className="font-semibold text-gray-900">{click.candidate_name}</p>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <a href={getEmailClientLink(click, template)} target="_blank" rel="noopener noreferrer" className="p-2 bg-blue-500 text-white hover:bg-blue-600 rounded-lg transition-colors">
                          <Send size={16} />
                        </a>
                      </div>
                    </div>
